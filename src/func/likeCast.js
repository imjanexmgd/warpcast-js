...
export default async (token, castHash, idempotencyKey) => {
  try {
    const { data } = await axios.put(
      'https://client.warpcast.com/v2/cast-likes',
      {
        castHash: castHash,
      },
      {
    ...
    loggerSuccess(
      `user ${data.result.like.reactor.username} success give react like at ${data.result.like.timestamp}`
    );
    return data;
  } catch (error) {
    const reason = error.response.data.errors[0].message || 'unkown';
    loggerFailed(`failed like cast ${castHash} reason ${reason}`);
  }
};
